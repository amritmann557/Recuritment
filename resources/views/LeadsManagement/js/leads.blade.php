...
$('body').on('click','#edituploadDesignDetails',function(){
...
<!-- Website Content -->
$('body').on('click','#viewWebsiteEnquiryButton',function()
	{
	var id=$(this).attr('rel');
    $.ajax({
    url: "{{ route('viewWebsiteLeads') }}",
    type: "get",
    data: 
	{
        "_token": "{{ csrf_token() }}",
...
$('body').on('click','#updateStatusButton',function(){
	var id=$(this).attr('rel');
    $.ajax({
    url: "{{ route('getWebsiteLeadStatus') }}",
    type: "get",
    data: 
	{
        "_token": "{{ csrf_token() }}",
